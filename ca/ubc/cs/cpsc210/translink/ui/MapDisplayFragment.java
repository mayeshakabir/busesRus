...
public class MapDisplayFragment extends Fragment implements MapEventsReceiver, IMyLocationConsumer {
    ...
    private Stop nearest; //MADE THIS
            ...
            for (Route route : s.getRoutes()) {
                ...
                for (RoutePattern routePattern : route.getPatterns()) {
                    ...
                    for (LatLon latlon : routePattern.getPath()) {
                        if ((previous != null) && Geometry.rectangleIntersectsLine(northWest, southEast, previous, latlon)) {
                            List<GeoPoint> geoPoints = new ArrayList<>();
                            GeoPoint prevGeoPoint = new GeoPoint(previous.getLatitude(), previous.getLongitude());
                            geoPoints.add(prevGeoPoint);
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
    ...
    private void markStops() {
        ...
        for (Stop stop: StopManager.getInstance()) {
            ...
            if (Geometry.rectangleContainsPoint(northWest, southEast, locn)) {
    ...
    private void updateMarkerOfNearest(Stop nearest) {
        ...
        for (Marker m: stopClusterer.getItems()) {
            ...
            Stop stop = (Stop)m.getRelatedObject();
            if (nearest == stop) {
